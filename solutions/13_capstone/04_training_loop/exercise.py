# Solution -- 13.04 Capstone: the training loop
...
def train_step(
    ...
    for group in optimizer.param_groups:
        group["lr"] = lr
    # zero_grad BEFORE backward: .grad accumulates across calls (08.02), and
    # a stale gradient from the previous batch would be added to this one.
    optimizer.zero_grad()
    _, loss = model(x, y)
    loss.backward()
    # Clip AFTER backward and BEFORE step. The rescaling is applied to the
    # gradients the optimiser is about to consume (09.10).
