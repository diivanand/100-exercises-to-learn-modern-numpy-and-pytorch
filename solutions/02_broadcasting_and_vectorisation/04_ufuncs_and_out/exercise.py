...
def running_total(xs: np.ndarray) -> np.ndarray:
    ...
    # Every ufunc carries `.accumulate`; np.cumsum is this one with a name.
    return np.add.accumulate(xs)


def multiplication_table(n: int) -> np.ndarray:
    """The (n, n) table t[i, j] = (i + 1) * (j + 1)."""
    # `.outer` applies the ufunc to every pair. np.outer exists too, but
    # `.outer` works for ANY ufunc: np.maximum.outer, np.subtract.outer, ...
...
def normalised(xs: np.ndarray) -> np.ndarray:
    ...
    # The output buffer must be able to hold the result. np.empty_like(xs)
    # copies xs's dtype, and an integer buffer cannot receive a float result:
    # ufuncs refuse to cast 'same_kind' downward and raise. Say the dtype.
    out = np.empty(xs.shape, dtype=np.float64)
...
def add_into(total: np.ndarray, delta: np.ndarray) -> None:
    ...
    # `out=total` writes straight into the caller's buffer. `total + delta`
    # would build a new array and leave the caller's untouched (03.04).
