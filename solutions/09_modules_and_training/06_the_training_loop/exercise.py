# Solution -- 09.06 The training loop
...
def make_eval_step(model: nn.Module, loss_fn: Callable[..., torch.Tensor]) -> Step:
    ...
    def eval_step(x: torch.Tensor, y: torch.Tensor) -> float:
        # Two switches, and they are independent: eval() changes what Dropout
        # and BatchNorm do; inference_mode() stops autograd recording (and
        # saving every activation for a backward pass that will never come).
        model.eval()
