# =============================================================================
#  13.05 -- Capstone: generation
# =============================================================================
#
#  The last exercise. A trained model gives, for a prefix, a score for every
#  possible next character. Generation is a loop: score the prefix, choose a
#  character, append it, repeat. Every choice the loop makes is a knob you
#  have used on a chatbot without knowing it (Kneusel PDL ch. 18 explains
#  temperature; Cautaerts ch. 14 samples with top-k):
#
#   * GREEDY: take the argmax every time. Deterministic, and prone to loops
#     ("the the the").
#
#   * TEMPERATURE: divide the LOGITS by T before the softmax. T -> 0 sharpens
#     the distribution towards the argmax; T = 1 samples from the model's own
#     distribution; T > 1 flattens it. It must be applied to the logits:
#     dividing the probabilities after the softmax changes nothing, because
#     the sampler only sees their ratios.
#
#   * TOP-K: keep the k largest logits and set the rest to -inf, so that the
#     softmax gives them zero probability, then sample among the survivors.
#     The threshold is per ROW of the batch -- shape (B, 1), so it broadcasts
#     against (B, V).
#
#   * THE CONTEXT WINDOW: the model has position embeddings for `context`
#     positions and no more. Once the sequence is longer than that, only the
#     last `context` tokens are fed in. Forget this and generation crashes
#     the moment it passes the window.
#
#   * REPRODUCIBILITY: `torch.multinomial` draws from the global RNG unless
#     you hand it a `torch.Generator` (12.04). Pass one through, and the same
#     seed gives the same text on the same machine.
#
#  `@torch.inference_mode()` (08.03) wraps the whole loop: no graph, no grad,
#  and tensors it returns are inference tensors, which is why the tests replay
#  the model under inference_mode as well.
#
#  The final test trains the model for 500 steps (about 4 s on a laptop CPU)
#  and checks that the greedy continuation of the corpus's first line is the
#  corpus's first line. When it passes, run this file's `generate` on the
#  GPU machine with a bigger corpus and a bigger model: nothing in it needs
#  to change except `device`.
#
#  TASK
#    Crop to the context window, apply the temperature to the logits, fix the
#    top-k threshold shape, and pass the generator to the sampler.
#
#  RUN IT
#    ./npt test 13_05
#
...
def next_token_logits(
    ...
    # TODO: feed the model only the last `model.context` tokens. The position
    # table has no rows beyond that.
    logits, _ = model(idx)
    logits = logits[:, -1, :]
    if top_k is not None:
        # TODO: the k-th largest value per row has shape (B,); comparing it
        # against (B, V) logits either fails to broadcast or compares against
        # the wrong row. Keep it as a (B, 1) column: `.values[:, -1:]`.
...
def generate(
    ...
    for _ in range(max_new_tokens):
        if temperature == 0.0:
            ...
            # TODO: the temperature has to divide the LOGITS before the
            # softmax. Dividing the probabilities afterwards scales every
            # entry equally, and multinomial only looks at their ratios.
            probs = torch.softmax(
                next_token_logits(model, idx, temperature, top_k), dim=-1
            )
            probs = probs / temperature
            # TODO: pass `generator=generator`, or the draw comes from the
            # global RNG and is not reproducible.
